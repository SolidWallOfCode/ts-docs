...
Both scripts depend on a file name :swoc:git:`atc/openssl.cnf` which is defined by OpenSSL. This is the base
content, which should be tweaked as needed for local conditions.

.. literalinclude:: openssl.cnf

The script :swoc:git:`atc/ssh-gen-ca.sh` can be used to generate the root certificate.

.. literalinclude:: ssh-gen-ca.sh

Once the root certificate has been generated, deployment certificates must be generated.
Client certificates used for mutual TLS can be generated with the script :swoc:git:`atc/ssh-gen-cert.sh`.

.. literalinclude:: ssh-gen-cert.sh

Because Traffic Ops is the central hub for the components, those other components validate that the
...
data. The :swoc:git:`ssh-gen-cert-with-host.sh` script requires a host name as an argument which is embedded
in to the SAN data in way that validates with ATC components. The script uses the domain
"solidwallofcode.com" which should be changed to the domain being used for the CDN infrastructure.

.. literalinclude:: ssh-gen-cert-with-host.sh
